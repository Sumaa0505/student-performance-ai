...
def predict_proba_sim(label: str):
    """Simulasi distribusi probabilitas berdasarkan kelas prediksi."""
    if label == "High":
        return {"High": 0.78, "Medium": 0.16, "Low": 0.06}
    elif label == "Medium":
        return {"High": 0.22, "Medium": 0.59, "Low": 0.19}
    else:
        return {"High": 0.05, "Medium": 0.23, "Low": 0.72}
...
# ============================================================
    ...
    if not hasattr(model, "feature_importances_"):
        st.warning("Model ini tidak menyediakan atribut `feature_importances_`. Fitur ini tersedia pada model berbasis pohon keputusan (Random Forest, Gradient Boosting, dll.)")
        st.stop()
...
# ============================================================
    ...
    if st.button("⚖️  Bandingkan Kedua Skenario", use_container_width=True, type="primary"):
        ...
        hasil_a  = predict(build_input_dataframe(raw_a))
        hasil_b  = predict(build_input_dataframe(raw_b))
        proba_a  = predict_proba_sim(hasil_a)
        proba_b  = predict_proba_sim(hasil_b)
...
# ============================================================
    ...
    with tab_m:
        ...
        fases = [
            ...
            ("4. Modeling",
             "Model dilatih menggunakan algoritma berbasis pohon keputusan "
             "(misalnya Random Forest atau Gradient Boosting). Hyperparameter disetel "
